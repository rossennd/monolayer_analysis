...
###########################################################
# B. Define Experiment-Specific Analysis Variables Here ---------------------------------------------------------------

# Define data_dir as the filepath to a folder containing .csv files for each imaged well named like 'X##Results.csv' (ex. J03Results.csv). 
# Row 1 of each CSV must contain column names. Subsequent rows contain data for each imaging frame. 
# Columns 1-3 can be named arbitrarily but must contain the frame #s, frame label (arbitrary), and background ROI intensity values respectively.
# Subsequent columns must be named uniquely like 'Mean(ROI###)' (ex. Mean(ROI011)) and contain EC cell ROI intensity values.
# Results will be saved in a new folder ('Analysis') in the parent directory of data_dir. Each time the script is run for a given data_dir, a new timestamped results folder will be created in 'Analysis'.
data_dir <- "/Users/janedoe/Desktop/imaging_data/250101/intensitydata" #directory containing csv files
expt_name <- "250101" #arbitrary unique experiment ID. Used to label wells, cells, and results files.



#define experimental conditions (aka wellgroups)
wellgroup_names <- c(
  "Vehicle",
  "Drug A",
  "Drug B",
...
#assign wells to each wellgroup (vectors must be the same order and length as wellgroup_names)
wellgroup_defs <- list(
  c("A01", "A02", "B01", "B02"), # Vehicle
  c("A03", "A04", "B03", "B04"), # Drug A
  c("A05", "A06", "B05", "B06"), # Drug B
  c("A07", "A08", "B07", "B08") # Drug C
)

names(wellgroup_defs) <- wellgroup_names

control_wellgroup <- "Vehicle" #define wellgroup used draw horizontal line on stats plots.

drug_times = c(20, 50) #define drug addition frames to show on traceplot (leave vector empty for no lines)

baseline_period <- c(0, 20) #Start and end frames for normalization baseline period.

# Define starting and ending frames for each analysis period and choose which to analyze. 
## The vector length defines the number of time periods created. 'P1, 'P2', and 'P3' are created here.
## When using relative metrics, the periods in periods_to_analyze are each analyzed in comparison to 'P1'. 
## When using absolute metrics, the periods in periods_to_analyze are each analyzed independently.
## When quantifying transient excitation, periods_to_analyze is ignored and 'P1' defines analysis baseline period, 'P2' defines transient effect period and "P3" defines sustained effect period. 
time_period_defs <- data.frame(StartTime =  c(10, 40, 70), 
                               EndTime =    c(20, 50, 80)
)
...
###########################################################-
###########################################################
# D. Open, normalize, and compile raw intensity data and perform period measurements--------------------------------------------------------------------
...
###########################################################
# # E. Combine previously processed experimental data for analysis. (COMMENT OUT SECTION UNLESS COMBINING DATA FROM MULTIPLE EXPERIMENTS) ###############
# #Running this section allows you to take previously generated Time series and period measurement data from multiple experiments and combine them for pooled analysis.
# #Once sections A-C have been run at least once, the script can be run from this section onward.
# 
# top_output_dir <- "/Users/janedoe/Desktop/imaging_data/multi_experiment_analysis/" #Define filepath for an existing folder to save multi-experiment analysis.
# expt_name <- "DrugsABC" # Define the multi-experiment analysis ID. A folder with this name will be created within top_output_dir and each run of the script will create a new results folder within it. 
# 
# # Specify the filepath to the previously generated Results folders you would like to combine data from. The appropriate data files will automatically be extracted from these folders.
# input_dirs <- c(
#   "/Users/janedoe/Desktop/imaging_data/250101/Analysis/e250101_t250111-235959", #Experiment 1 Results folder
#   "/Users/janedoe/Desktop/imaging_data/250102/Analysis/e250102_t250116-235959",  #Experiment 2 results folder
#   "/Users/janedoe/Desktop/imaging_data/250103/Analysis/e250103_t250118-235959"  #Experiment 3 results folder
# )
# 
# #choose which of the previously defined wellgroups you want to analyze from these experiments 
# wellgroup_names <- c(
#   "Vehicle",
#   "Drug A",
#   "Drug C"
...
# 
# #Here, additional experiment analysis variables defined in section B can be overwritten.
# # Trace plot parameters
# y_limits = c(-.2, .7)
# x_limits = c(20, 80) # NA is full range
# 
...
# --- Create Well Trace Grid (COMMENT OUT IF USING SECTION E) -------------------------------------------------------------
plot_list <- NULL
...
###########################################################
# G. Run Wellgroup GEE and create Statsplots ---------------------------------------------------------
...
# --- Relative metrics for comparing baseline and treatment periods (COMMENT OUT SECTION WHEN MEASURING SINGLE-PERIOD ACTIVITY).  ---------------------------------------------------------
...
# # --- Absolute metrics for measuring baseline actvity. (COMMENT OUT SECTION WHEN COMPARING ACTIVITY BETWEEN PERIODS). ---------------------------------------------------------
# 
# # Absolute Maximum
# metric <- "max"
# for (period in periods_to_analyze) {
...
###########################################################
# # H. Quantify number of cells exhibiting transient excitation (COMMENT OUT OR DONT RUN IF NOT USING)  #######
# # --- Identify "Responders" - ---------------------------------------------------
